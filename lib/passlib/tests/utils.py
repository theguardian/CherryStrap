...
import tempfile
import time
from lib.passlib.exc import lib.passlibHashWarning
from lib.passlib.utils.compat import PY27, PY_MIN_32, PY3, JYTHON
import warnings
...
from lib.passlib.tests.backports import TestCase as _TestCase, catch_warnings, skip, skipIf, skipUnless
from lib.passlib.utils import has_rounds_info, has_salt_info, rounds_cost_values, \
                          classproperty, rng, getrandstr, is_ascii_safe, to_native_str, \
                          repeat_string, tick
from lib.passlib.utils.compat import b, bytes, iteritems, irange, callable, \
...
class TestCase(_TestCase):
    ...
    * suite of methods for matching against warnings
    ...
    def require_stringprep(self):
        ...
        from lib.passlib.utils import stringprep
        if not stringprep:
            from lib.passlib.utils import _stringprep_missing_reason
...
class HandlerCase(TestCase):
    ...
    def _enable_backend_case(cls, backend):
        ...
        if handler.name == "bcrypt" and backend == "builtin" and TEST_MODE("full"):
            ...
            return None
        from lib.passlib.utils import has_crypt
    ...
    def prepare_salt(self, salt):
        ...
        if self.fuzz_salts_need_bcrypt_repair:
            from lib.passlib.utils import bcrypt64
    ...
    def test_63_large_secret(self):
        ...
        from lib.passlib.exc import PasswordSizeError
        from lib.passlib.utils import MAX_PASSWORD_SIZE
    ...
    def test_64_forbidden_chars(self):
        ...
        if isinstance(chars, bytes):
            from lib.passlib.utils.compat import iter_byte_chars
